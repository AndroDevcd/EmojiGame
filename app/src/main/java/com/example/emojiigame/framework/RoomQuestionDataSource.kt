...
import com.example.emojiigame.data.QuestionDataSource
import com.example.emojiigame.domain.Question
import com.example.emojiigame.framework.db.EmojiiGameDatabase
import kotlin.random.Random


class RoomQuestionDataSource(val context: Context) : QuestionDataSource {
    ...
    override suspend fun getAll(): List<Question>  {
        var dbQuestions : ArrayList<Question>? =
            questionDao.getAllQuestions()?.map { it -> Question(it.question, it.answer) } as ArrayList<Question>
    ...
    override suspend fun setupQuestions() : ArrayList<Question> {
        ...
        ques.add(Question("Name the drama show: ⛰️⛰️", "Twin peaks"));
        ques.add(
            Question(
